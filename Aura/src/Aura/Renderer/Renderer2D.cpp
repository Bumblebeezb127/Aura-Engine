...
namespace Aura
{
	...
	void Renderer2D::Init() 
	{
		...
		s_Data.QuadVertexArray = Aura::VertexArray::Create();
		...
		uint32_t* quadVertexIndices = new uint32_t[s_Data.MaxIndices];

		uint32_t offset = 0;
		for(uint32_t i = 0; i < s_Data.MaxIndices; i += 6) {
			quadVertexIndices[i + 0] = offset + 0;
			quadVertexIndices[i + 1] = offset + 1;
			quadVertexIndices[i + 2] = offset + 2;
			quadVertexIndices[i + 3] = offset + 2;
			quadVertexIndices[i + 4] = offset + 3;
			quadVertexIndices[i + 5] = offset + 0;
			offset += 4;
		}


		Ref<IndexBuffer> quadIB = IndexBuffer::Create(quadVertexIndices, s_Data.MaxIndices * sizeof(uint32_t));
		s_Data.QuadVertexArray->SetIndexBuffer(quadIB);
		delete[] quadVertexIndices;
		...
		//memset(s_Data.TextureSlots.data(), 0, sizeof(uint32_t) * s_Data.MaxTextureSlots);
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const glm::vec4& color)
	{
		...
		//glm::mat4 transform = translate(glm::mat4(1.0f), position) * glm::scale(glm::mat4(1.0f), { size,1.0f });
